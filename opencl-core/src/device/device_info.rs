...
use crate::cl::{cl_get_info5, ClObject, ClObjectError, ClPointer};
...
use crate::device::{Device, DeviceError, DeviceType};
...
impl Device {
    ...
    pub fn platform(&self) -> Output<Platform> {
        self.get_info(DeviceInfo::Platform)
            .and_then(|ret| unsafe { ret.into_created_wrapper() })
    }
}
