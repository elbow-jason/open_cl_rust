...
impl From<cl_device_id> for Device {
    fn from(device_id: cl_device_id) -> Self {
        Device(device_id)
    }
}

impl From<Device> for cl_device_id {
    fn from(device: Device) -> cl_device_id {
        device.cl_object()
    }
}
...
impl From<Device> for Vec<cl_device_id> {
    fn from(device: Device) -> Vec<cl_device_id> {
        vec![device.cl_object()]
    }
}
...
impl Device {
    pub fn cl_object(&self) -> cl_device_id {
        self.0
    }

    pub fn new(device_id: cl_device_id) -> Device {
        Device(device_id)
    }

    pub fn create_context(&self) -> Output<Context> {
        let context_id = cl_create_context(&self.cl_object())?;
    ...
    pub fn count_by_type(platform: &Platform, device_type: DeviceType) -> Output<u32> {
        cl_get_device_count(&platform.cl_object(), device_type.bits())
    }

    pub fn all_by_type(platform: &Platform, device_type: DeviceType) -> Output<Vec<Device>> {
        let ids = cl_get_device_ids(&platform.cl_object(), device_type.bits())?;
        Ok(ids.into_iter().map(|id| Device::new(id)).collect())
    }

    pub fn get_info(&self, info: DeviceInfo) -> Output<String> {
        cl_get_device_info(&cl_device_id::from(self), info as cl_device_info)
    }
