    ...
    where F: FnMut(&Device, &Context, &CommandQueue)
{
    ...
    for p in platforms.iter() {
        let devices = p.all_devices().unwrap();
...
mod hl {
    ...
    fn simple_kernel() {
        ...
        super::test_all(&mut |device, ctx, queue| {
            let prog = ctx.create_program_with_source(src.to_string()).unwrap();
            prog.build_on_one_device(&device).unwrap();
            

            let k = prog.fetch_kernel("test").unwrap();
            let v1: Vec<isize> = vec![1];
            let mem1 = ctx.create_read_write_buffer::<isize>(v1.len()).unwrap();
            println!("mem1 {:?}", mem1);
            
            let work_size = v1.len();
            let work: Work = Work::new(work_size);
            let mut v_buffer = HostBuffer::new(v1);

            let () = queue.write_buffer(&mem1, &v_buffer, EventList::empty(), None).unwrap();
            let () = k.set_arg(0, &mem1).unwrap();

            let _event = queue.sync_enqueue_kernel(k, work, EventList::empty())
                .unwrap_or_else(|error| {
                    format!("Failed to unwrap sync_enqueue_kernel result: {:?}", error);
                    None
                })
                .expect("Failed to unwrap sync_enqueue_kernel ClEvent it was None");
            
            let () = queue.read_buffer(&mem1, &mut v_buffer, EventList::empty(), None).unwrap();
            let v2: Vec<isize> = v_buffer.into_data();
            expect!(v2.len(), 1);
            expect!(v2[0], 1);
...
//             queue.enqueue_async_kernel(&ctx, &k, 1isize, None, ()).wait();
...
//             queue.enqueue_async_kernel(&ctx, &k, (3isize, 3isize), None, ()).wait();
